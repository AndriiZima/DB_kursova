...
{
    public partial class Request_19 : Form
    {
        public Request_19()
        {
            InitializeComponent();
        ...
        {
            DB db = new DB();
            string showDate = "SELECT e.full_name, 'Немає ефірів сьогодні' AS comment FROM employees e WHERE e.id_work_position = 1 AND e.id_employees NOT IN( SELECT id_employee FROM program_ether WHERE DATE(date_holding) = CURDATE()) UNION SELECT e.full_name, '' AS comment FROM employees e WHERE e.id_work_position = 1 AND e.id_employees IN( SELECT id_employee FROM program_ether WHERE DATE(date_holding) = CURDATE())";
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int y1 = marginTop;
            // Друк назви запиту
            string queryName = "Список співробітників з коментарем: \"Немає ефірів сьогодні\"";
