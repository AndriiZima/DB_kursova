...
{
    public partial class Directory_competitions : Form
    {
        public Directory_competitions()
        {
            InitializeComponent();
            Load_Date();
        }
        ...
        {
            DB db = new DB();
            string showDate = "select * from directory_competitions";
        ...
        {
            DB db = new DB();
            MySqlCommand insertCommand = new MySqlCommand("INSERT INTO `directory_competitions` (`name_competition`, `date_holding`, `id_guest`) VALUES (@name_competition, @date_holding, @id_guest);", db.getConnection());
            insertCommand.Parameters.Add("@name_competition", MySqlDbType.VarChar).Value = textBox1.Text;
            insertCommand.Parameters.Add("@date_holding", MySqlDbType.VarChar).Value = textBox2.Text;           
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                int idToDelete = (int)selectedRow.Cells["id_directory_competitions"].Value;
                ...
                using (MySqlConnection connection = new MySqlConnection("server=localhost;uid=Biba;pwd=aboba123;database=coolcompany"))
                {
                    ...
                    string query = "DELETE FROM directory_competitions WHERE id_directory_competitions = @id";
