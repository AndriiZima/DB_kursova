...
{
    public partial class Request_14 : Form
    {
        public Request_14()
        {
            InitializeComponent();
        ...
        {
            DB db = new DB();
            string showDate = "SELECT e.full_name, COUNT(pe.id_program_ether) AS num_program_ether FROM employees e LEFT JOIN program_ether pe ON e.id_employees = pe.id_employee GROUP BY e.full_name HAVING COUNT(pe.id_program_ether) > (SELECT AVG(num_program_ether) FROM (SELECT COUNT(pe.id_program_ether) AS num_program_ether FROM employees e LEFT JOIN program_ether pe ON e.id_employees = pe.id_employee GROUP BY e.id_employees) AS subquery)";
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int y1 = marginTop;
            // Друк назви запиту
            string queryName = "Прізвища працівників та кількість проведених ефірів";
