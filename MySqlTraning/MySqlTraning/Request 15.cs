...
{
    public partial class Request_15 : Form
    {
        public Request_15()
        {
            InitializeComponent();
        ...
        {
            DB db = new DB();
            string showDate = "SELECT e.full_name FROM employees e LEFT JOIN program_ether pe ON e.id_employees = pe.id_employee AND pe.date_holding >= DATE_ADD(DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY), INTERVAL 0 DAY) AND pe.date_holding < DATE_ADD(DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY), INTERVAL 7 DAY) WHERE pe.id_program_ether IS NULL AND e.id_work_position = 1";
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int y1 = marginTop;
            // Друк назви запиту
            string queryName = "Хто не проводив ефір на цьому тиждні?";
